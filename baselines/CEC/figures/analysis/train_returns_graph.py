"""Per-layout training-return line plot, fetched directly from a wandb run."""
...
SAVE_DIR = Path(__file__).parent.parent / "results" / "train_returns_graph"
...
def fetch_history(entity: str, project: str, run_id: str):
    ...
    keys = ["env_step"] + [f"train_returns/{name}" for name in LAYOUT_NAMES]
...
# Plotting
# ──────────────────────────────────────────────
def plot_train_returns(history, run_name: str, x_axis: str, smooth_window: int, out_path: Path):
    cols = [f"train_returns/{name}" for name in LAYOUT_NAMES]
    ...
    for name, color in zip(LAYOUT_NAMES, LAYOUT_COLORS):
        ax.plot(history[x_axis], smoothed[f"train_returns/{name}"],
    ...
    ax.set_title(f"[{variant}] Train Returns per Layout — {run_name}")
...
def main():
    ...
    out_path = SAVE_DIR / f"train_returns_{model_name}_{args.run_id}_{args.x_axis}.png"
    plot_train_returns(history, run_name, args.x_axis, args.smooth_window, out_path)
