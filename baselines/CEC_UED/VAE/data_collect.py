...
def make_train(config, update_step=0):
    ...
    def train(rng, model_params=None, update_step=0):
        ...
        #######################################################################################################
        # Optional: save initial obsv, env_state (numpy for portability)

        save_dir = config.get("DATA_SAVE_DIR", "collected_data")
        os.makedirs(save_dir, exist_ok=True)
        to_save = jax.tree_util.tree_map(lambda x: np.asarray(x), (obsv, env_state))
        with open(os.path.join(save_dir, "initial_reset_obsv_env_state.pkl"), "wb") as f:
...
def main(config):
    ...
    filepath = f"/app/baselines/CEC_UED/VAE/dataset"
    print(f"Working on: \n{filepath}\n")
