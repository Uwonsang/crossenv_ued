...
DECOUPLED_POLICY_VALUE_METRIC_NAMES = (
    "policy_grad_norm",
    "weighted_value_grad_norm",
)

DECOUPLED_ENVIRONMENT_GRADIENT_METRIC_NAMES = (
...
DECOUPLED_FEATURE_RANK_NAMES = (
    ("policy", "policy_penultimate"),
    ("value", "value_penultimate"),
)


def empty_decoupled_policy_value_metrics(dtype=jnp.float32):
    return {
        name: jnp.asarray(jnp.nan, dtype=dtype)
        for name in DECOUPLED_POLICY_VALUE_METRIC_NAMES
    }


def empty_decoupled_environment_gradient_metrics(dtype=jnp.float32):
    return {
        name: jnp.asarray(jnp.nan, dtype=dtype)
        for name in DECOUPLED_ENVIRONMENT_GRADIENT_METRIC_NAMES
    }


def compute_decoupled_policy_value_norms(
    *,
    network,
    params,
    initial_hstate,
    ...
    """Measure policy and value norms over the full disjoint networks."""
...
def make_train(
    ...
    def train(rng, model_params=None, resume_runner_state=None):
        ...
        # Actor and critic own independent Adam states and clipping coefficients.
        ...
        def _update_step(update_runner_state, unused):
            ...
            if stiffness_enabled:
                ...
                _, permutation_rng, _ = jax.random.split(post_rollout_rng, 3)
            ...
            if stiffness_enabled:
                ...
                feature_rank_metrics = jax.lax.cond(
                    ...
                    lambda _: compute_pooled_feature_rank_metrics(
                        ...
                        feature_names=DECOUPLED_FEATURE_RANK_NAMES,
                ...
                policy_value_metrics = jax.lax.cond(
                    run_stiffness,
                    lambda _: compute_decoupled_policy_value_norms(
                        network=network,
                        params=original_params,
                        initial_hstate=policy_value_hstate,
                    ...
                    lambda _: empty_decoupled_policy_value_metrics(),
                    operand=None,
                )

                def _compute_decoupled_environment_gradient_metrics(_):
                    all_metrics = compute_static_grid_conditioned_gradients(
                        network=network,
                        params=original_params,
                ...
                environment_gradient_metrics = jax.lax.cond(
                    run_stiffness,
                    _compute_decoupled_environment_gradient_metrics,
                    lambda _: empty_decoupled_environment_gradient_metrics(),
                    operand=None,
                )
            else:
                stiffness_metrics = empty_paper_stiffness_metrics()
                static_unique_count = jnp.asarray(jnp.nan, dtype=jnp.float32)
                policy_value_metrics = empty_decoupled_policy_value_metrics()
                feature_rank_metrics = empty_pooled_feature_rank_metrics(
                    DECOUPLED_FEATURE_RANK_NAMES
                )
                environment_gradient_metrics = (
                    empty_decoupled_environment_gradient_metrics()
                )
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(
                        ...
                        return total_loss, (
                            value_loss, loss_actor, entropy, ratio,
                            approx_kl, clip_frac,
                        )
...
def main(config):
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            entity=config["ENTITY"],
            project=config["PROJECT"],
            tags=["DECOUPLED_IPPO", "RNN", "SP", "UNIQUE_STATIC_GRID"],
            config=config,
            mode=config["WANDB_MODE"],
            name=f"IPPO_decoupled_gradient_{layout_name}_seed{config['SEED']}"
        )
