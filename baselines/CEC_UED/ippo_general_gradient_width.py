...
# Parameter-count control for the separate-trunk ablation.
#
# With the default Overcooked configuration (FC_DIM_SIZE=GRU_HIDDEN_DIM=256),
# the separate-trunk model has 5,270,919 parameters.  Widening both dimensions
# to 376 gives this shared-trunk model 5,275,097 parameters (0.079% more), the
# closest integer width when both dimensions are scaled together.
PARAMETER_MATCHED_WIDTH_SCALE = 376 / 256


def configure_parameter_matched_width(config):
    """Scale FC/RNN widths once while retaining the configured base widths."""
    if "WIDTH_BASE_FC_DIM_SIZE" not in config:
        config["WIDTH_BASE_FC_DIM_SIZE"] = config["FC_DIM_SIZE"]
        config["WIDTH_BASE_GRU_HIDDEN_DIM"] = config["GRU_HIDDEN_DIM"]

    config["FC_DIM_SIZE"] = round(
        config["WIDTH_BASE_FC_DIM_SIZE"] * PARAMETER_MATCHED_WIDTH_SCALE
    )
    config["GRU_HIDDEN_DIM"] = round(
        config["WIDTH_BASE_GRU_HIDDEN_DIM"] * PARAMETER_MATCHED_WIDTH_SCALE
    )
    return config
...
def make_train(
    ...
    config = configure_parameter_matched_width(config)
    # env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"])
...
def main(config):
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            ...
            config=config,
            mode=config["WANDB_MODE"],
            name=f"CEC_gradient_width_{layout_name}_seed{config['SEED']}"
        )
