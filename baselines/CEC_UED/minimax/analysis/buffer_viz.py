...
def load_pickle(file_path: str):
    with open(file_path, "rb") as f:
        data = pickle.load(f)
    plr_buffer = jax.device_put(jax.tree.map(jnp.array, data["plr_buffer"]))
    return plr_buffer

def slice_levels(buffer, n=100):
    out= {}
    for k, v in buffer["levels"].items():
        if v.ndim == 0:
            out[k] = v
            continue
        out[k] = v[:n]
...
def visualize_buffer(buffer, n = 100):
    save_dir = "/app/baselines/CEC_UED/minimax/analysis/buffer_viz"
    ...
    for idx, layout in tqdm(enumerate(level_layout)):
        ...
        imageio.imwrite(os.path.join(save_dir, f"buffer_frame_{idx:03d}.png"), frame)


def main():
    file_path = (
        "/app/ckpts/ippo/overcooked/cramped_room_9/ikTrue/reset_all/lr-20260407-124217/plr_buffer/plr_buffer_step_000.pkl"
    )
