"""
Modified-wall CEC adaptation of the IDAAC PopArt implementation.
...
def modified_wall_ckpt_root(config):
    return f"ckpts/idaac_pop/{config['ENV_NAME']}/modified_wall/{get_wall_map_dir_name(config)}"
...
def load_xp_partner_params(config):
    ...
    pattern = (
        ...
        f"seed{partner_seed}_ckpt0_improved_pop_updates*.pkl"
    )
...
def main(config):
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            ...
            config=config,
            mode=config["WANDB_MODE"],
            name=(f"CEC_IDAAC_POPART_modified_wall_{layout_name}_seed{config['SEED']}")
        )
