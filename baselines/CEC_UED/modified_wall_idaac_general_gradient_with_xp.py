...
def initialize_environment(config):
    if config["ENV_NAME"] == "ToyCoopNoPink":
        ...
        @scan_tqdm(heldout_num)
        def gen_held_out_toycoop(runner_state, unused):
            (i,) = runner_state
            state = env.custom_reset_fn(jax.random.key(i), random_reset=True)
            return (i + 1,), (state.agent_pos, state.goal_pos, state.wall_map)

        _, heldout = jax.lax.scan(
            gen_held_out_toycoop, (0,), jnp.arange(heldout_num), heldout_num
        )
        fixed_states = [
        ...
        env.held_out_agent_pos = jnp.concatenate([
            heldout[0], jnp.stack([state.agent_pos for state in fixed_states])
        ])
        env.held_out_goal_pos = jnp.concatenate([
            heldout[1], jnp.stack([state.goal_pos for state in fixed_states])
        ])
        env.held_out_wall_map = jnp.concatenate([
            heldout[2], jnp.stack([state.wall_map for state in fixed_states])
        ])
...
@hydra.main(version_base=None, config_path="config", config_name="ippo_overcooked_CEC_dual_destination_idaac_with_xp")
def main(config):
