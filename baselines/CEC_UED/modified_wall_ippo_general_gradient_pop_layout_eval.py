...
@hydra.main(version_base=None, config_path="config", config_name="ippo_overcooked_CEC_dual_destination_popart_layout_eval")
def main(config):
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            ...
            config=config,
            mode=config["WANDB_MODE"],
            name=f"CEC_POPART_LAYOUT_EVAL_modified_wall_{layout_name}_seed{config['SEED']}" if config["ENV_NAME"] == "ToyCoop" else f"CEC_gradient_pop_layout_eval_{layout_name}_seed{config['SEED']}"
        )
