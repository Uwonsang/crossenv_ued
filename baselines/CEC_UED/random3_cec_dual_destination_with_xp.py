...
def make_modified_wall_env(config):
    ...
    return ToyCoopNoPinkRandom3(**env_kwargs)
...
def fixed_toy_states_for_layout_eval(config):
    return [
        ToyCoopNoPink(**make_fixed_toy_env_kwargs(config, map_name)).custom_reset_fn(
...
def initialize_environment(config):
    ...
    if config["ENV_NAME"] == "overcooked":
        ...
        held_out = generate_unique_held_out_states(
            env,
            toy_heldout_num,
            fixed_states,
        ...
        ho_agent_pos = jnp.concatenate([held_out.agent_pos, fixed_agent_pos], axis=0)
        ho_goal_pos = jnp.concatenate([held_out.goal_pos, fixed_goal_pos], axis=0)
        ho_wall_map = jnp.concatenate([held_out.wall_map, fixed_wall_map], axis=0)
        ...
        print(
            ...
            f"{toy_heldout_num} procedural + {len(fixed_states)} fixed = "
            f"{ho_agent_pos.shape[0]}"
        )
...
def make_train(config, update_step=0, xp_partner_params=None):
    ...
    eval_layout_names = EVAL_LAYOUTS_9 if is_overcooked else get_toy_layout_names(config)
    ...
    def train(rng, model_params=None, update_step=0):
        ...
        def _update_step(update_runner_state, unused):
            ...
            if len(eval_envs) > 0:
                ...
                def _skip_eval(_):
                    out = {n: jnp.array(jnp.nan, dtype=jnp.float32) for n in eval_layout_names}
...
@hydra.main(version_base=None, config_path="config", config_name="cec_toycoop_nopink_random3_65k")
def main(config):
