"""
Modified-wall CEC adaptation of IDAAC with training-time cross-play.
...
from jaxmarl.environments.toy_coop.toy_coop_no_pink_random3 import (
    ToyCoopNoPinkRandom3,
    generate_unique_held_out_states,
)
...
import representation_metrics as representation_metrics_module
...
def _memory_efficient_feature_metrics(features, cutoff=0.01):
    feature_matrix = features.reshape((-1, features.shape[-1]))
    gram = (
        feature_matrix @ feature_matrix.T
        if feature_matrix.shape[0] <= feature_matrix.shape[1]
        else feature_matrix.T @ feature_matrix
    )
    singular_values = jnp.sqrt(jnp.maximum(jnp.linalg.eigvalsh(gram), 0.0))[::-1]
    scale_metrics = representation_metrics_module.feature_scale_metrics(
        feature_matrix, singular_values
    )
    threshold = 1 - cutoff
    singular_sum = jnp.sum(singular_values)
    distribution = singular_values / singular_sum
    distribution = jnp.where(distribution == 0, 1, distribution)
    ...
    rank_metrics = {
        "feature_rank": jnp.sum(
            singular_values / jnp.sqrt(feature_matrix.shape[0]) > cutoff
        ).astype(jnp.float32),
        "effective_rank_vetterli": jnp.exp(
            -jnp.sum(distribution * jnp.log(distribution))
        ).astype(jnp.float32),
        "srank_kumar": (
            jnp.sum(jnp.cumsum(singular_values) < threshold * singular_sum) + 1
        ).astype(jnp.float32),
        "approximate_rank_pca": (
            jnp.sum(jnp.cumsum(squared) < threshold * jnp.sum(squared)) + 1
        ).astype(jnp.float32),
        "matrix_rank": jnp.sum(singular_values > rank_tolerance).astype(jnp.float32),
    }
...
def get_wall_map_dir_name(config):
    ...
    name = map_name if map_name != "mixed" else "mixed_" + "_".join(get_toy_layout_names(config))
    ckpt_tag = str(config.get("CKPT_TAG", "")).strip()
    return f"{name}_{ckpt_tag}" if ckpt_tag else name
...
def make_modified_wall_env(config, map_name=None, evaluation=False):
    allowed = {
        "random_reset", "max_steps", "check_held_out", "debug",
        "partial_obs", "incentivize_strat", "map_name",
        "randomize_walls", "num_random_walls",
        "heldout_ignore_agent_order",
    }
    ...
    if evaluation:
        ...
        env_kwargs.pop("randomize_walls", None)
        env_kwargs.pop("num_random_walls", None)
        env_kwargs.pop("heldout_ignore_agent_order", None)
    if env_kwargs["map_name"] == "mixed":
        env_kwargs["layout_names"] = get_toy_layout_names(config)
    env_cls = ToyCoopNoPink if evaluation else ToyCoopNoPinkRandom3
...
def initialize_environment(config):
    if config["ENV_NAME"] == "ToyCoopNoPink":
        ...
        heldout_num = int(config.get("TOY_HELDOUT_NUM", 100))
        fixed_states = [
            make_modified_wall_env(config, name, evaluation=True).custom_reset_fn(
                jax.random.PRNGKey(0), random_reset=False
            )
            for name in get_toy_layout_names(config)
        ]
        heldout = generate_unique_held_out_states(
            env,
            heldout_num,
            fixed_states,
            seed=int(config.get("TOY_HELDOUT_SEED", 0)),
        )
        env.held_out_agent_pos = jnp.concatenate([
            heldout.agent_pos, jnp.stack([state.agent_pos for state in fixed_states])
        ])
        env.held_out_goal_pos = jnp.concatenate([
            heldout.goal_pos, jnp.stack([state.goal_pos for state in fixed_states])
        ])
        env.held_out_wall_map = jnp.concatenate([
            heldout.wall_map, jnp.stack([state.wall_map for state in fixed_states])
        ])
        print(
            "ToyCoop held-out states: "
            f"{heldout_num} procedural + {len(fixed_states)} fixed = "
            f"{env.held_out_agent_pos.shape[0]}"
        )
...
def make_train(
    ...
    config.setdefault("GRAD_CONFLICT_WINDOW_STEPS", config.get("GRAD_CONFLICT_STEPS", 32))
    config.setdefault("GRAD_CONFLICT_CHUNK_SIZE", 64)
    # env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"])
    ...
    def train(
        rng, model_params=None, resume_runner_state=None,
    ):
        # INIT NETWORK
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                if is_overcooked:
                    ...
                    layout_id = jax.vmap(_classify_layout_jax)(pre_maze_map[:, 4:13, 4:13, 0])
                else:
                    layout_id = jax.vmap(
                        lambda wall_map: _classify_toy_layout_jax(wall_map, toy_layout_wall_maps)
            ...
            def _calculate_gae(traj_batch, last_val):
                ...
                return advantages, targets, td_errors
            ...
            # Value targets and critic outputs are both on the reward scale.
            target_stats = compute_value_diagnostics(
                ...
                normalized_target_prefix="target_value",
            )
            ...
            def _compute_layout_gradient(_):
                ...
                return compute_layout_gradient_metrics(
                    ...
                    value_targets=(
                        targets[:layout_gradient_window_steps]
                    ),
                    layout_ids_full=(
            ...
            def callback(metric):
                ...
                if "eval_returns" in metric:
                    if np.isfinite(float(metric["eval_returns"]["mean"])):
                        eval_last = {
                            ...
                            **{_ln: float(metric["eval_returns"][_ln]) for _ln in layout_names},
                        }
                ...
                if len(layout_names) > 0:
                    ...
                    for t, e in np.argwhere(ep_done):
                        label = layout_names[int(layout_ids[t, e])]
...
@hydra.main(version_base=None, config_path="config", config_name="dcec_toycoop_nopink_random3_65k")
def main(config):
    ...
    if config["ENV_NAME"] == "ToyCoopNoPink":
        filepath_base += f"/modified_wall/{get_wall_map_dir_name(config)}"
    elif config["ENV_NAME"] == "overcooked":
    ...
    if wandb_resume_id:
        ...
        wandb.init(
            ...
            tags=["CEC", "IDAAC", "RNN", "SP"],
            config=config,
            mode=config["WANDB_MODE"],
            name=(f"CEC_IDAAC_modified_wall_{layout_name}_seed{config['SEED']}")
        )
    ...
    train_jit(
        rng, model_params, resume_runner_state,
    )
