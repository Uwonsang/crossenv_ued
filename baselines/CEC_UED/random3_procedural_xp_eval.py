...
MODEL_SPECS = {
    "ippo_empty": {
        ...
        "train_map": "empty",
        "fixed_layouts": ["empty"],
        "wandb_group": "XP_IPPO_256_EMPTY_x_IPPO_256_EMPTY",
    },
    "ippo_wall_a": {
        "family": "ippo",
        "algorithm": "IPPO",
        ...
        "wandb_group": "XP_IPPO_256_WALL_A_x_IPPO_256_WALL_A",
    },
    "e3t_empty": {
        "family": "e3t",
        "algorithm": "E3T",
        "train_map": "empty",
        "fixed_layouts": ["empty"],
        "wandb_group": "XP_E3T_256_EMPTY_x_E3T_256_EMPTY",
    },
    "e3t_wall_a": {
        "family": "e3t",
        "algorithm": "E3T",
        "train_map": "wall_a",
        "fixed_layouts": ["wall_a"],
        "wandb_group": "XP_E3T_256_WALL_A_x_E3T_256_WALL_A",
    },
    "cec_random3": {
        "family": "cec_random3",
        "algorithm": "CEC",
        "train_map": "random_3_walls",
        "fixed_layouts": ["empty", "wall_a"],
        "wandb_group": "XP_RANDOM3_CEC_256",
    },
    "dcec_random3": {
        "family": "dcec_random3",
        "algorithm": "DCEC",
        "train_map": "random_3_walls",
        "fixed_layouts": ["empty", "wall_a"],
        "wandb_group": "XP_RANDOM3_DCEC_256",
    },
}
    ...
    if model_group == "cec_random3":
        return [
            str(
                ...
                / config["ENV_NAME"]
                / "modified_wall"
                / "random_3_walls_with_xp_numenv256"
                / "ikTrue"
                / "reset_all"
                / "cec_layout_eval"
                / "**"
...
def generate_state_bank(config):
    ...
    env = ToyCoopNoPinkRandom3(
        max_steps=int(config["ENV_KWARGS"]["max_steps"]),
        random_reset=True,
        check_held_out=False,
        partial_obs=bool(config["ENV_KWARGS"]["partial_obs"]),
        incentivize_strat=int(config["ENV_KWARGS"]["incentivize_strat"]),
        map_name="random_3_walls",
        randomize_walls=True,
        num_random_walls=int(config["ENV_KWARGS"]["num_random_walls"]),
        heldout_ignore_agent_order=True,
    )
    procedural = generate_unique_held_out_states(
        env,
        int(config["NUM_PROCEDURAL_TASKS"]),
        fixed_states,
        seed=int(config["HELDOUT_SEED"]),
    )
    fixed_agent_pos = jnp.stack([state.agent_pos for state in fixed_states])
    fixed_goal_pos = jnp.stack([state.goal_pos for state in fixed_states])
    fixed_wall_map = jnp.stack([state.wall_map for state in fixed_states])
    agent_pos = np.asarray(jax.device_get(jnp.concatenate([
        procedural.agent_pos, fixed_agent_pos
    ])))
    goal_pos = np.asarray(jax.device_get(jnp.concatenate([
        procedural.goal_pos, fixed_goal_pos
    ])))
    wall_map = np.asarray(jax.device_get(jnp.concatenate([
        procedural.wall_map, fixed_wall_map
    ])))
    procedural_layouts = np.asarray(
        ["random_3_walls"] * int(config["NUM_PROCEDURAL_TASKS"])
    )
    ...
    return {
        ...
        "goal_pos": goal_pos,
        "wall_map": wall_map,
        "procedural_layouts": procedural_layouts,
...
def load_or_create_state_bank(config):
    ...
    for index in range(num_procedural):
        agents = bank["agent_pos"][index]
        goals = bank["goal_pos"][index]
        agent_order = np.lexsort((agents[:, 0], agents[:, 1]))
        goal_order = np.lexsort((goals[:, 0], goals[:, 1]))
        signature = (
            agents[agent_order].tobytes(),
...
def make_network(config, spec):
    ...
    elif family == "cec_random3":
        from random3_cec_dual_destination_with_xp import (
            ActorCriticRNN,
            ScannedRNN,
        )
    ...
    elif family == "dcec_random3":
        from random3_dcec_dual_destination_with_xp import ActorCriticRNN
...
@hydra.main(
    ...
    config_name="random3_procedural_xp",
)
def main(config):
    ...
    if checkpoint_num_envs != 256:
        raise ValueError(f"Random3 XP requires CKPT_NUM_ENVS=256, got {checkpoint_num_envs}")
