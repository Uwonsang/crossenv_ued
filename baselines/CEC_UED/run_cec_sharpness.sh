...
#
# Run from anywhere:
#   bash baselines/CEC_UED/run_cec_sharpness.sh
#
# Common overrides:
#   CUDA_VISIBLE_DEVICES=0 \
#   PYTHON_BIN=/path/to/python \
#   TRAINING_ROLLOUT_SIZES="128 256" \
#   SEEDS="0 1" \
#   OUTPUT=results/sharpness.json \
#   bash baselines/CEC_UED/run_cec_sharpness.sh
#
# Extra arguments are forwarded to measure_cec_sharpness.py:
#   bash baselines/CEC_UED/run_cec_sharpness.sh --dry-run
...
TRAINING_ROLLOUT_SIZES="${TRAINING_ROLLOUT_SIZES:-32 256}"
...
OUTPUT="${OUTPUT:-sharpness_cec_32_256.json}"

read -r -a model_args <<< "${MODELS}"
read -r -a training_rollout_args <<< "${TRAINING_ROLLOUT_SIZES}"
read -r -a seed_args <<< "${SEEDS}"
read -r -a epsilon_args <<< "${EPSILONS}"

echo "CEC sharpness evaluation"
echo "  models:                 ${MODELS}"
echo "  training rollout sizes: ${TRAINING_ROLLOUT_SIZES}"
echo "  seeds:                  ${SEEDS}"
...
exec "${PYTHON_BIN}" -m baselines.CEC_UED.measure_cec_sharpness \
    --models "${model_args[@]}" \
    --training-rollout-sizes "${training_rollout_args[@]}" \
    --seeds "${seed_args[@]}" \
