...
echo "Running mixed layout-eval CEC/PopArt retrain + multi-map XP"
echo "  train map: $TRAIN_MAP"
echo "  train layouts: config layout_names"
...
if [[ "$TRAIN_CEC" == "true" ]]; then
  echo
  echo "===== Train mixed-layout CEC with layout eval ====="
  for seed in $SEEDS; do
    echo "----- CEC layout-eval ${TRAIN_MAP} seed ${seed} -----"
...
if [[ "$TRAIN_POPART" == "true" ]]; then
  echo
  echo "===== Train mixed-layout PopArt CEC with layout eval ====="
  for seed in $SEEDS; do
    echo "----- CEC_POPART layout-eval ${TRAIN_MAP} seed ${seed} -----"
...
echo
echo "Mixed layout-eval CEC/PopArt retrain + multi-map XP finished."
