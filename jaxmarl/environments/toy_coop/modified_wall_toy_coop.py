...
class ModifiedWallToyCoop(MultiAgentEnv):
    ...
    LAYOUTS = {
        ...
        "wall_b": ("CCGCC", "CCBCC", "ACBCA", "CCBCC", "CCGCC"),
    ...
    def _parse_layout(self, map_name):
        ...
        if len(rows) != self.height or any(len(row) != self.width for row in rows):
            raise ValueError(f"Layout {map_name!r} must be {self.height}x{self.width}.")
        ...
        if len(goal_pos) != 2:
            raise ValueError(f"Layout {map_name!r} must contain exactly two G tokens.")

        return (
            jnp.array(wall_rows, dtype=bool),
            jnp.array(agent_pos, dtype=jnp.int32),
            jnp.array(goal_pos, dtype=jnp.int32),
        )
    ...
    def get_obs(self, state: State) -> Dict[str, chex.Array]:
        ...
        # 5x5x5 observation: ego position, other agent position, goal positions, auxiliary goals, walls
        ...
        obs_0 = obs_0.at[:, :, 4].set(state.wall_map.astype(obs_0.dtype))
    ...
    def observation_space(self, agent_id: str = "") -> spaces.Box:
        ...
        return spaces.Box(0, 1, (self.height * self.width * 5,))


ToyCoop = ModifiedWallToyCoop
