...
def render_tile(agent_idx, is_goal, is_wall, agent_dir_idx, tile_size=TILE_PIXELS):
    ...
    def render_wall(img):
        ...
        return fill_coords(img, fn, jnp.array([90, 90, 90]))

    img = jnp.where(is_wall, render_wall(img), img)
...
def render_grid(agent_pos, goal_pos, wall_map, agent_dir_idx, height=5, width=5, tile_size=TILE_PIXELS):
